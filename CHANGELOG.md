...
### Changed
- **`step_simulation(observe_prims=...)` reports `position_world`, not `position`.** The value was already world in every branch, but it kept the unqualified name `get_prim_info` deliberately removed — so the two tools taught opposite contracts, and the field an agent uses most to measure motion was the unqualified one. Both tools now name the frame.
- **Tool descriptions trimmed 16%** (23,350 → 19,730 characters, ~5.8k → ~4.9k tokens). Every tool docstring sits in the agent's context on every call, and the accumulated explanations of what once broke had begun to crowd out the instructions. Rationale moved to code comments; the contract — fields returned, ordering rules, actionable limits — is unchanged, and `get_prim_info` / `load_environment` now document the warning keys they return. A test caps the total so it cannot creep back.
- **`get_prim_info`'s `transform.position` is replaced by `position_local` and `position_world`** ([#39](https://github.com/whats2000/isaacsim-mcp-server/issues/39)). It held the *parent-relative* pose with nothing saying so, while `actual_size` in the same response was world-space. Robot links are where it bit — `/World/Franka/fr3_hand_tcp` — because Isaac's FR3 is a flat hierarchy rooted at the articulation, so a robot at the default origin makes the two frames coincide and the tool look correct; put the robot anywhere else and the same call silently returned base-relative coordinates. Naming one field `position` and the other `position_world` would have kept the trap, so there is no bare `position`: callers must choose a frame. `position_world_source` is `"usd"` or `"physics"`. On Newton the Fabric pose now fills `position_world` instead of overwriting `position` — that overwrite meant one field carried a world value on Newton and a local one everywhere else; when that measured pose is served, `position_local_warning` says the local value is still the authored spawn pose. `rotation` and `scale` remain local, matching what `transform_object` writes.
...
- **`create_camera(target=...)` aimed from the wrong frame.** With no explicit position it read the camera's own pose as the eye point and passed it to a look-at against a **world** target, so a camera nested under a transformed parent was aimed from a parent-relative point — and still produced a rotation, so it failed silently. The orientation was wrong the same way: the look-at produces a **world** rotation and it was written as a **local** xform op, so under a rotated parent the camera missed a world-origin target by 74.7° while a translate-only parent looked exact. Both the eye point and the orientation are now resolved in the right frame. Found while fixing #39.
